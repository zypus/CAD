...
public class OutputComponent extends JComponent {
	...
	public List<Segment> createPoly(List<Shape> shape) {
		...
		for (Shape line : shape) {
			poly.add(new Segment(new Point(((Line2D)line).getX1(),((Line2D)line).getY1()), new Point(((Line2D)line).getX2(),((Line2D)line).getY2())));
		}
		return poly;
	}
	...
	public OutputComponent() {
		...
		MouseAdapter mouseAdapter = new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				boolean isInside = e.getPoint().getX() >= 0 && e.getPoint().getY() >= 0 && e.getPoint().getX() <= myWidth && e.getPoint().getY() <= myHeight;
				if (drawLines) {
					...
					for (int s = 0; s < shapes.size(); s++) {
						for (Shape line : shapes.get(s)) {
							...
							double mouseDistance = ((Line2D)line).getP1().distance(e.getPoint())+((Line2D)line).getP2().distance(e.getPoint());
							if (mouseDistance <= length + 1 ) {
								changed = true;
								if (((e.getModifiers() & MouseEvent.CTRL_MASK) == MouseEvent.CTRL_MASK)) {
									if (selectedShapes2.size() > 0 && !((e.getModifiers() & MouseEvent.SHIFT_MASK) == MouseEvent.SHIFT_MASK)) {
										selectedShapes2.clear();
									}
...
//									}
									...
									if (selectedShapes1.size() > 0 &&!((e.getModifiers() & MouseEvent.SHIFT_MASK) == MouseEvent.SHIFT_MASK)) {
										selectedShapes1.clear();
									}
//									if (selectedShapes2.contains(s)) {
//										selectedShapes2.remove(s);
...
//								System.out.println(polygons.get(selectedShapes1.get(i))+" "+polygons.get(selectedShapes1.get(j)));
								intersections.add(PolyChecker.findIntersectionPoints(polygons.get(selectedShapes1.get(i)), polygons.get(selectedShapes1.get(j))));
							}
						}
					}
				}
...
//											}
								...
								if (intersecting) {
									...
									if (inside2) {
										mouseText += "outside, area: "+(PolyChecker.area(polygons.get(hover))-PolyChecker.area(merged2));
									}
									else {
										mouseText += "outside";
									}
			...
			private boolean showIndicator(MouseEvent e) {
				if (currentShape != null && Math.abs(e.getX()-((Line2D)currentShape.get(0)).getX1()) < 10 && Math.abs(e.getY()-((Line2D)currentShape.get(0)).getY1()) < 10) {
					if (shapeStartIndicator == null) {
						shapeStartIndicator = new Ellipse2D.Double(((Line2D)currentShape.get(0)).getX1()-8, ((Line2D)currentShape.get(0)).getY1()-8, 16, 16);
					}
					return true;
				}
...
//					Path2D path = new Path2D.Double();
					...
					counter++;
					if (line == shapes.get(s).get(shapes.get(s).size()-1) && !((Line2D)line).getP2().equals(((Line2D)shapes.get(s).get(0)).getP1())) {
...
//				g2d.setPaint(c);
			...
			if (shapeCount < ((currentShape == null) ? scroll.getComponentCount() : scroll.getComponentCount() -1)) {
				if (!((Line2D)shapes.get(s).get(shapes.get(s).size()-1)).getP2().equals(((Line2D)shapes.get(s).get(0)).getP1())) {
					((JLabel)scroll.getComponent(shapeCount)).setText("open -> Length: "+(int)PolyChecker.parameter(polygons.get(shapeCount)));
				}
				else {
					((JLabel)scroll.getComponent(shapeCount)).setText("closed -> Length: "+(int) PolyChecker.parameter(polygons.get(shapeCount))+" Area: "+(int)PolyChecker.area(polygons.get(shapeCount)));
				}
